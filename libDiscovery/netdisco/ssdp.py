    ...
    try:
        while sockets:
            ...
            for sock in ready:
                ...
                #print(entry.match_device_description(values=entry.values))
                #print(entry.values.get('location', '_NO_LOCATION')) #obtiene url
                checksame=0
                if lista:
                    for item in range(0, len(lista)):
                        if (entry.description.get('device') == lista[item]):
                            checksame=1
                            break
                    if checksame != 1:
